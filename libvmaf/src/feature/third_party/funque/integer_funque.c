...
fail:
    ...
    for(unsigned i=0; i<4; i++)
    {
        ...
        if (s->i_ref_dwt2out_vif.bands[i]) aligned_free(s->i_ref_dwt2out_vif.bands[i]);
        if (s->i_dist_dwt2out_vif.bands[i]) aligned_free(s->i_dist_dwt2out_vif.bands[i]);
    }
                   ...
                   unsigned index, VmafFeatureCollector *feature_collector)
{
    ...
    int16_t shift_val_2 = pow(2, 2*SPAT_FILTER_COEFF_SHIFT-SPAT_FILTER_INTER_SHIFT-SPAT_FILTER_OUT_SHIFT+4*DWT2_COEFF_UPSHIFT-2*DWT2_INTER_SHIFT-2*DWT2_OUT_SHIFT)*bitdepth_pow2; //2^13.99 
    ...
    err = compute_vif_funque(s->i_ref_dwt2out_vif.bands[0],s->i_dist_dwt2out_vif.bands[0] ,s->ref_dwt2out_vif.bands[0], s->dist_dwt2out_vif.bands[0], s->ref_dwt2out_vif.width, s->ref_dwt2out_vif.height, &vif_score_1, &vif_score_num_1, &vif_score_den_1, 9, 1, (double)5.0, shift_val_2);
    if (err) return err;
